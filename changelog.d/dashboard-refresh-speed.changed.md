- **Faster Apple dashboard refresh.** The App Store Connect queries now run
  ...
  instead of the full history. Group assignments made through the dashboard
  mark the build dirty so it is refetched. Pending-fragment authored dates
  are memoized like feature landings, a dead per-fragment first-parent log
  walk was dropped, and each refresh logs its timing to stderr.
