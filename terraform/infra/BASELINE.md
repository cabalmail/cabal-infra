...
| [`.checkov.baseline`](.checkov.baseline) | Per-resource grandfather of the 118 residual Checkov findings (41 ids). New findings fail. |
...
| ---- | ----- | ------------------- | --------- | ------------------------------- | -------- |
| Checkov | 243 | 76 (12 ids) | 118 (41 ids) | 14 (276, 51, 8, 341, 26, 27x3, 103, 74, 12 fixed; 111, 356, 2_18, 21 inline) | 0 |
...
- **CKV_AWS_150** (load balancer deletion protection, x2) - **fixed**. Both public load balancers now set `enable_deletion_protection = true`: the production mail NLB (`module.load_balancer.aws_lb.elb`, `modules/elb/main.tf`), where an accidental console or `terraform destroy` deletion would take email offline, and the dormant monitoring ALB (`module.monitoring.aws_lb.uptime`, `modules/monitoring/alb.tf`) for consistency. Deletion protection is an AWS-API guard (not a `lifecycle` meta-arg), so it would otherwise block the `destroy_terraform.yml` teardown of a non-prod environment; that workflow's strip step now also flips `enable_deletion_protection = true` to `false` in its throwaway working copy (matching how it already strips `lifecycle.prevent_destroy`), so a deliberate teardown still proceeds. Both `CKV_AWS_150` baseline entries removed; the id is fully cleared from the gate. **Teardown note (not a data-plane concern):** the next `destroy_terraform.yml` run against stage/development now depends on that strip step - if a future refactor renames the attribute, teardown will fail until the strip is updated.
- **CKV_AWS_18 / AWS-0089** (S3 server access logging) on the three content buckets - **fixed**. A new shared target bucket (`modules/s3_access_logs`, `cabal-s3-access-logs-<account>`: versioned, SSE-S3, 180-day lifecycle, public-access-blocked, with a delivery policy granting `logging.s3.amazonaws.com` PutObject scoped by `aws:SourceAccount` + the three source-bucket ARNs) now receives `aws_s3_bucket_logging` from `admin.<control_domain>` (modules/s3, prefix `admin/`), `www.<control_domain>` (modules/front_door, prefix `front-door/`), and `cache.<control_domain>` (modules/app, prefix `cache/`). The three `CKV_AWS_18` baseline entries are removed and `AWS-0089` leaves `.trivyignore`. The two buckets that would self-recurse - the new target bucket and the existing NLB access-log bucket (`modules/elb`) - carry an inline `#trivy:ignore:AVD-AWS-0089` (and the target bucket keeps the same `#checkov:skip=CKV_AWS_18` the NLB bucket uses). Collateral graph-renderer effect: adding the module changed checkov's variable rendering for one count-gated, dormant monitoring resource (`module.monitoring.aws_ecs_task_definition.cloudwatch_exporter`), which checkov stopped reporting `CKV_AWS_336` for, so that one stale baseline entry was dropped on regeneration (deterministic; its `_us_east_1` twin and the seven other `CKV_AWS_336` instances stay baselined, and the design-driven rationale in section 3 still covers it). This is the same checkov graph fragility documented in the OAC-migration and graph-check notes; monitoring is off in every environment, so the resource is dormant.
...
| CKV_AWS_115 (x9) | - | Lambda reserved concurrency - all Lambdas deliberately share one account-wide concurrency pool. Reserved concurrency both *guarantees and caps*: capping `api_call` (the API-Gateway user-facing hot path, which the React/Apple clients hit with many parallel envelope/message fetches) would throttle legitimate bursts, and reserving on the peripheral functions carves capacity out of the same pool `api_call` draws from - the exact harm the two most-recently-added Lambdas' inline skips already reject ("shared-pool concurrency is the point; a per-function reserve would starve the API lambdas", [`push_dispatch.tf`](modules/app/push_dispatch.tf) / [`push_token_gc.tf`](modules/app/push_token_gc.tf)). The nine flagged are the API function itself plus low-rate background/trigger invokers (`append_sent` SQS consumer, `process_dmarc` / `certbot` EventBridge Scheduler jobs, `check_invite` / `assign_osid` Cognito triggers) and the three dormant monitoring functions (`alert_sink` / `backup_heartbeat` / `healthchecks_iac`, `TF_VAR_MONITORING=false` everywhere). On a single-operator, low-volume system with no second tenant to protect, the runaway blast-radius the check guards against is negligible. Baselined per resource, so a new Lambda is still caught. Revisit if the account ever runs a genuinely bursty second workload that needs guaranteed carve-outs. |
