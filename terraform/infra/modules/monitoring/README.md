...
- **No fail2ban metrics**: fail2ban was removed from the mail-tier images entirely in 0.10.x (it had been commented out of every `supervisord.conf` since 0.7.0 and never ran). Login-rate limiting now lives in Dovecot's own knobs and, prospectively, the NLB/WAF; there is no fail2ban log stream to derive a metric from. See [docs/0.10.x/container-runtime-hardening-plan.md](../../../../docs/0.10.x/container-runtime-hardening-plan.md).
