...
module "admin" {
  source              = "./modules/app"
  control_domain      = var.control_domain
  user_pool_id        = local.pool.user_pool_id
  user_pool_client_id = local.pool.user_pool_client_id
  region              = var.aws_region
...
module "imap" {
  ...
  user_pool_arn    = local.pool.user_pool_arn
  region           = var.aws_region
  ports            = [143, 993]
  private_ports    = [25]
  cidr_block       = var.cidr_block
  ...
  client_id        = local.pool.user_pool_client_id
  user_pool_id     = local.pool.user_pool_id
