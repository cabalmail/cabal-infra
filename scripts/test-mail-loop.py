...
Each message gets a unique subject and body sampled from a frequency-ranked
3000-word English vocabulary using Zipf's law, producing a corpus with a
realistic mix of common and uncommon words for full-text-search UAT.
...
Optional env vars / args (env shown in parens):
  ...
  --seed                        random seed for reproducible corpus (default: random)
...
def _compute_zipf_cdf(n, s=1.0):
    """Cumulative distribution for Zipf(s) over ranks 1..n, normalized to [0, 1]."""
...
def _zipf_word(rng):
    return WORDS[bisect.bisect_left(_ZIPF_CDF, rng.random())]


def _make_sentence(rng):
    words = [_zipf_word(rng) for _ in range(rng.randint(6, 20))]
    words[0] = words[0].capitalize()
    punct = rng.choices((".", "?", "!"), weights=(92, 5, 3))[0]
    return " ".join(words) + punct


def _make_paragraph(rng):
    return " ".join(_make_sentence(rng) for _ in range(rng.randint(3, 6)))


def make_body(rng):
    return "\n\n".join(_make_paragraph(rng) for _ in range(rng.randint(1, 5)))


def make_subject(rng):
    words = [_zipf_word(rng) for _ in range(rng.randint(3, 7))]
    words[0] = words[0].capitalize()
    return " ".join(words)
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Send a synthetic test email on a fixed cadence.",
    ...
    p.add_argument(
        ...
        type=int,
        default=None,
        help="Random seed for reproducible corpus (default: system-random)",
    )
    return p.parse_args()


def build_message(sender, recipient, sequence, total, rng):
    subject_tail = make_subject(rng)
    body = make_body(rng)
    msg = EmailMessage()
    msg["Subject"] = f"test-mail-loop {sequence}/{total} - {subject_tail}"
    msg["From"] = sender
    msg["To"] = recipient
...
def main():
    ...
    for i in range(total):
        ...
        seq = i + 1
        rng = random.Random(seed_base + seq)
        msg = build_message(args.sender, args.recipient, seq, total, rng)
        try:
...
# --- Vocabulary ---------------------------------------------------------
# Top 3000 English words from Google's Trillion Word Corpus, ordered by
# frequency rank (rank 1 = most common). Sampled with Zipf's law in
# _zipf_word() to produce a realistic mix of common and uncommon tokens
# across many generated messages.
# Source: https://github.com/first20hours/google-10000-english
#   (google-10000-english-usa-no-swears.txt, top 3000 lines)
_WORDS_RAW = """\
the
of
and
...
WORDS = tuple(_WORDS_RAW.split())
_ZIPF_CDF = _compute_zipf_cdf(len(WORDS))
