...
- **iOS default-mail-app gate.** Discovered post-ship: iOS does not surface a `mailto:`-registered app in Settings → Apps → Mail → Default Mail App unless the app also carries the Apple-managed `com.apple.developer.mail-client` entitlement, granted case-by-case via Apple's default-app web form on developer.apple.com (replacing the older `default-app-requests@apple.com` route). macOS has no equivalent gate. The iOS half of Phase 5's user-facing benefit is therefore parked behind that request — code is in place, but the entitlement (and a regenerated provisioning profile) has to land before iOS users can pick Cabalmail. `apple/README.md` covers the request and rollout steps, including the four behavioral assertions the form requires.
