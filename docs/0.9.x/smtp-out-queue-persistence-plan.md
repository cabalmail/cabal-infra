...
- Persisting queues for `smtp-in` or `imap`. Inbound relay drops are bounced upstream, not queued for our retry; IMAP local delivery is synchronous to the EFS-backed mailstore.
  ...
  > **Erratum (2026-08-06):** The smtp-in half of this premise was wrong. Sendmail is store-and-forward: smtp-in takes custody at `250` *before* attempting the mailertable relay to imap, and a failed attempt (connection refused/timeout while the imap tier is mid-deploy) is a tempfail that queues in smtp-in's own `/var/spool/mqueue` — nothing is "bounced upstream," because upstream already got its `250`. On the ephemeral container filesystem that queue died with the task, silently dropping accepted mail whenever an imap delivery gap overlapped a smtp-in task replacement — observed on stage as never-backfilled gaps in a once-per-minute prod-to-stage mail loop across tier rolls. Corrected by applying this plan's pattern to smtp-in (PR #944). The imap half of the sentence stands.
