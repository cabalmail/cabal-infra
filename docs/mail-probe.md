...
- **`GET /list_messages -> 500` or a bodiless `502` on the first run after provisioning**: imap has not restarted since the `ci-probe` user was created, so Dovecot has no such user. Roll the imap tier (see [Provisioning](#provisioning)) and re-run. (A `502` is a crashed handler; the probe does not retry it, only the `503` of a maintenance window and the `504` of a timeout.)
