...
- **Single-host hotspot**: drain and replace the EC2:
  ...
  ```
- **Cluster-wide saturation**: scale the ASG up or move to a larger instance class. The monitoring services are memory-heavy, not CPU-heavy — if CPU is the bottleneck, expect mail-tier load. Check the [IMAPAuthFailureSpike](./imap-auth-failure-spike.md) signal and the Dovecot auth-failure logs — with no public IMAP listener since #778/#779, a spike there is the API Lambdas failing to authenticate or an in-VPC source attempting logins, not an internet brute force.
