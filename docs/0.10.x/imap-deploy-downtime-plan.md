...
> **Erratum (2026-08-06):** "Well-trodden ground" overstates this path, and step 1 above should not be built on. The `mail_nfs_*` settings were always best-effort — upstream's own characterization was that they work 99% of the time, which is a lower corruption rate, not an invariant — and Dovecot 2.4 removed them entirely as never having worked reliably, alongside removing director from the community edition. What is well-trodden is multi-backend *with per-user session affinity*; free-running concurrent writers on one mailbox remain unsafe in every Dovecot configuration. If overlap-during-deploy is ever pursued, the more promising direction is the one opened by the private mail-plane replumb: with effectively all IMAP traffic arriving as short-lived per-request Lambda connections against one internal endpoint (`imap.cabal.internal`), an atomic endpoint flip at cutover bounds dual-writer exposure to in-flight requests — a far weaker requirement than user-aware balancing, though still with real blast radius (a `dovecot-uidlist` race renumbers UIDs, poisoning the S3 message cache and client caches) and deserving its own plan.
