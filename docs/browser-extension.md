...
- **Mint + copy.** The detector will miss some sign-up forms (a bare `type="text"` field with an opaque generated name, for instance), and chasing every one is not the plan — the toolbar popup carries the escape hatch instead. "Mint + copy address" creates a fresh address on the chosen apex domain, labelled with the current tab's hostname unless the user edits the label, and puts it on the clipboard to paste into the form. There is no submission for the extension to observe, so the address is created confirmed, not pending. The label prefill comes from the content script in the tab's top frame rather than from a `tabs` or `activeTab` grant, which keeps the store-listing permission surface unchanged; on a page the content script does not run in, the label is simply blank.
