...
| 7. Private-link handoff | **Extension + redirector page complete**; Apple-client menu row not started (unblocked by the OQ9 embed — `getStateOfSafariExtension` can now gate it) |
| 8. Platform targets & distribution | **Partial.** Safari sign-in verified end-to-end (2026-08-30) and the macOS App Store upload lands cleanly and attaches to the branch's TestFlight group (2026-08-31, after three rounds of App Store validation: appex layout, host-app icon, manifest `icons`). Chrome listing is in its first review. **Embedded (OQ9)**: the macOS mail app embeds the extension as `CabalmailMacWebExtension` with the control domain handed over via the App Group; first signed build uploaded and attached to the `stage` TestFlight group 2026-09-03 (build 1788453877), after one App ID capability fix. Live handshake click-test in Safari still owed. The standalone host still ships during the transition. iOS/iPadOS (8.3, now via the iOS mail app) and parity testing (8.5) not started |
