...
| 4. Form detection | **Engine complete**, tuned against a real-site corpus: 22 captured fixtures across eight languages (2026-08-31) plus the fixer's additions; the corpus test judges the form the content script would actually pick. Weights moved only where the corpus demanded (underscore path terms, the heading walk, registration-shaped field sets, the legal-agreement signal). Growth toward ~110 and the strip-vs-keep decision on fixture styles remain open |
...
| 8. Platform targets & distribution | **Shipped** (1.11.0, 2026-09-04). Both Safari hosts are the mail apps themselves — `CabalmailMacWebExtension` and `CabalmailWebExtension` — signed, uploaded, and attached to the `prod` TestFlight groups; the "Open in Private Window" row is click-verified in prod. The standalone host still ships alongside; its retirement is undecided. Chrome listing remains in its first Web Store review. Parity testing (8.5) and visionOS are the only unstarted items |
