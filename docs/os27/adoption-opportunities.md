...
|---|---|---|---|---|
| Swipe actions in any container | `.swipeActionsContainer()` brings swipe actions to any view in a scroll container, no longer `List`-only. | Native swipe-to-archive/delete on our custom index-addressed virtualized message list, without the compromises that sank the SwipeRow attempt. Also worth re-testing the iPad leading-swipe-in-`NavigationSplitView` won't-fix under the new machinery. | Wait | |
