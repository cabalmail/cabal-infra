...
- Everything CI runs, in CI's order: `cd linux && cargo xtask ci` — fmt, clippy, kit tests, xtask tests, app tests, `cargo deny check`, stopping at the first failure. The last step needs `cargo-deny` (`cargo install --locked cargo-deny@0.20.2`); without it a whole-gate run warns and carries on, while CI and `--step supply-chain` both fail, and `workflow_contract.rs` fails if `linux.yml` and `lint.yml` install different versions. Run it before every push; `linux.yml` runs the same steps by name (`cargo xtask ci --step <name>`, one per job) rather than restating the commands in YAML, and `xtask/tests/workflow_contract.rs` fails if a step has no job or a job names a step that does not exist. `cargo xtask sync-vendored` materializes the composer's marked/turndown bundles (gitignored) from `react/admin/node_modules`, mirroring the Apple flow. `cargo xtask package arch` builds and namcap-lints the Arch package from `linux/packaging/arch/PKGBUILD` — an Arch machine only, and it builds `HEAD` rather than the working tree, because makepkg clones the checkout. `smoke` and `fixtures` are declared but land in Phase 2; asking for one prints the work item that owns it
...
- `cabalmail-kit` must reach no GTK, libadwaita, or WebKit crate at any depth — `xtask/tests/kit_purity.rs` asks cargo for the resolved tree and fails on one. A transitive GUI dependency would make the kit's tests need a display server, which is the single property the test story rests on
