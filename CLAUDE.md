...
### No hard-wrapping in GitHub-posted text

Anything posted to GitHub — issue bodies and comments, PR bodies, reviews, and comments — must not contain hard line breaks inside paragraphs. Write each paragraph as one long line and let the renderer wrap it. The maintainer reads these threads in email, where hard-wrapped paragraphs re-wrap at a different width and become very hard to track. Blank lines between paragraphs, lists, tables, and code fences are structure, not wrapping — those are fine. This rule is about GitHub-posted prose only; files in the repository (docs, changelog fragments) keep their own conventions.
...
Versioned subdirectories of `docs/` (e.g. `docs/0.4.0/`, `docs/0.7.0/`, `docs/0.9.x/`) are forward-looking plans for the corresponding roadmap version - design proposals written before or during implementation. A multi-phase plan carries its own progress: a `## Progress` table near the top and a `**Status:**` line under each work item, updated in the same PR as the work, so the next session reads where things stand instead of reconstructing it from git log. Once a feature ships, its as-implemented documentation lives at the top level of `docs/`, not inside the version directory. When you write operator-facing or reference documentation for something that has already shipped, put it in `docs/<topic>.md` and link it from the relevant index (`docs/operations.md`, `docs/setup.md`, etc.). Leave the version directory alone; it is part of the historical planning record. One exception: when a plan's claim is later proven wrong, add a dated erratum blockquote (`> **Erratum (YYYY-MM-DD):** ...`) immediately after the paragraph or bullet containing the falsified claim - at the top of the file if the whole doc is invalidated, never as a trailing section (windowed reads miss it). Never rewrite or delete the original text. When an erratum contradicts its surrounding text, the erratum is the corrected record - trust it.
...
- Lint all: `cd lambda/api && pylint --rcfile .pylintrc _shared/*.py */function.py push_dispatch/apns.py push_dispatch/fcm.py` (covers the shared modules, every handler, and the handler-sibling modules the `*/function.py` glob misses)
...
- Everything CI runs, in CI's order: `cd linux && cargo xtask ci` — fmt, clippy, kit tests, xtask tests, app tests, the kit's coverage floor, `cargo deny check`, stopping at the first failure. The last two steps need `cargo-llvm-cov` and `cargo-deny`, at the versions `linux/README.md` pins; without one a whole-gate run warns and names that version, while CI and `--step <name>` both fail, and a run against a *different* version than the pin is refused the same way — a distribution package satisfies "on PATH" and can read different rules than CI. `workflow_contract.rs` fails if the two workflows, `xtask/src/ci.rs`, and the README stop naming one version for either tool. Run it before every push; `linux.yml` runs the same steps by name (`cargo xtask ci --step <name>`, one per job) rather than restating the commands in YAML, and `xtask/tests/workflow_contract.rs` fails if a step has no job or a job names a step that does not exist. `cargo xtask sync-vendored` materializes the composer's marked/turndown bundles (gitignored) from `react/admin/node_modules`, mirroring the Apple flow. `cargo xtask package arch` builds and namcap-lints the Arch package from `linux/packaging/arch/PKGBUILD` — an Arch machine only, and it builds `HEAD` rather than the working tree, because makepkg clones the checkout; it also writes a `.SRCINFO` generated from the *unstaged* PKGBUILD, since the staged one points its git source at the local checkout. `cargo xtask smoke` installs a built package and launches it, asserting the installed binary reaches a main window — it needs root and installs into the root filesystem, so run it inside a container (`podman run --rm -v "$PWD:/repo:ro" -w /repo/linux archlinux:base-devel …`), never on your own machine. `fixtures` is declared but lands in Phase 3 with the API client; asking for it prints the work item that owns it
...
- The reader's `WebKitWebView` runs with `enable_javascript(false)`. This is the analog of the React reader's script-less iframe sandbox, and it is what neutralizes script in received mail — there is no HTML sanitizer behind it. Do not enable JavaScript on the reader view. (The composer's WebView is a different view with different rules; the prohibition is about the one that renders mail somebody else wrote.) The reader lands in Phase 4, which also owns the CI check that enforces this
- `cabalmail-kit` must reach no GTK, libadwaita, GDK, GSK, or WebKit crate at any depth — `xtask/tests/kit_purity.rs` asks cargo for the resolved tree and fails on one. A transitive GUI dependency would make the kit's tests need a display server, which is the single property the test story rests on
- The app crate builds the `cabalmail` binary
- CI: `linux.yml` runs one `cargo xtask ci` step per job. `clippy` and the widget tests run inside an `ubuntu:24.04` container — that is where the API floor is enforced, so a call needing GTK 4.16 fails `app-build` and leaves the format job green. System packages come from `linux/packaging/deps/<distro>.txt` via `.github/scripts/install-linux-deps.sh` (apt for `ubuntu`, pacman for `arch`), the same lists the PKGBUILD's dependency arrays and the Phase 8 Debian packaging read. `package-arch` and `smoke` are the two jobs that do not run a `ci` step: the first runs `cargo xtask package arch` in an `archlinux:base-devel` container as an unprivileged build user, since makepkg refuses to run as root, and uploads the package and its `.SRCINFO`; the second downloads that artifact, installs it into a clean container as root, and runs `cargo xtask smoke`. Both are held to running something by `workflow_contract.rs`, which also fails if the artifact is uploaded under one name and downloaded under another. `supply-chain` runs `cargo deny check` against `linux/deny.toml`; it is the one job whose result can turn red without anything in the repository changing, because a new advisory against a dependency is exactly what it exists to report. `coverage` runs `cargo-llvm-cov` over the kit against a line floor — a whole-crate floor, so it catches a broad regression rather than one untested module
...
| `codeql.yml` | PRs (per-language path filters), pushes to `stage`/`main`, weekly schedule | CodeQL code scanning as a checked-in workflow (GitHub default setup is off). On a PR only the languages whose sources changed are analysed; pushes and the schedule analyse all four (`actions`, `python`, `javascript-typescript`, `rust`). `codeql-gate` is the one stable check name to require. |
