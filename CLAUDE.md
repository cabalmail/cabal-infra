...
- Everything CI runs, in CI's order: `cd linux && cargo xtask ci` — fmt, clippy, kit tests, xtask tests, app tests, `cargo deny check`, stopping at the first failure. The last step needs `cargo-deny`, at the version `linux/README.md` pins; without it a whole-gate run warns and names that version, while CI and `--step supply-chain` both fail, and `workflow_contract.rs` fails if the two workflows, `xtask/src/ci.rs`, and the README stop naming one version. Run it before every push; `linux.yml` runs the same steps by name (`cargo xtask ci --step <name>`, one per job) rather than restating the commands in YAML, and `xtask/tests/workflow_contract.rs` fails if a step has no job or a job names a step that does not exist. `cargo xtask sync-vendored` materializes the composer's marked/turndown bundles (gitignored) from `react/admin/node_modules`, mirroring the Apple flow. `cargo xtask package arch` builds and namcap-lints the Arch package from `linux/packaging/arch/PKGBUILD` — an Arch machine only, and it builds `HEAD` rather than the working tree, because makepkg clones the checkout. `smoke` and `fixtures` are declared but land in Phase 2; asking for one prints the work item that owns it
...
- CI: `linux.yml` runs one `cargo xtask ci` step per job. `clippy` and the widget tests run inside an `ubuntu:24.04` container — that is where the API floor is enforced, so a call needing GTK 4.16 fails `app-build` and leaves the format job green. System packages come from `linux/packaging/deps/<distro>.txt` via `.github/scripts/install-linux-deps.sh` (apt for `ubuntu`, pacman for `arch`), the same lists the PKGBUILD's dependency arrays and the Phase 8 Debian packaging read. `package-arch` is the one job that does not run a `ci` step: it runs `cargo xtask package arch` in an `archlinux:base-devel` container as an unprivileged build user, since makepkg refuses to run as root. `supply-chain` runs `cargo deny check` against `linux/deny.toml`; it is the one job whose result can turn red without anything in the repository changing, because a new advisory against a dependency is exactly what it exists to report. Smoke and coverage are still to come; the workflow names the work item that owns each
