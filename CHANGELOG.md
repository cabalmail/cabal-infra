...
- Phase 2 of the 0.7.0 monitoring & alerting stack — heartbeat monitoring for the scheduled jobs that Phase 1's black-box probes can't see (cron-style Lambdas, EventBridge schedules, the in-container reconfigure loop, GitHub Actions). Gated by the same `var.monitoring` flag — no separate switch. Adds a self-hosted Healthchecks ECS service plus six per-job heartbeats wired through SSM Parameter Store.
...
- Heartbeat instrumentation across six scheduled jobs, all using the same `HEALTHCHECK_PING_PARAM` env-var → SSM-fetch → `urllib` ping pattern with a `value.startswith('http')` guard so placeholder values short-circuit cleanly:
  ...
  - `.github/workflows/terraform.yml` — new `heartbeat-ping` step at the end of the apply job, gated on `success() && vars.TF_VAR_MONITORING == 'true'`. Resolves the URL from `/cabal/healthcheck_ping_terraform_weekly` via `aws ssm get-parameter`, case-matches on the http(s) prefix, fires `curl -fsS -m 5`. The URL is captured into a shell variable rather than logged, sidestepping the GitHub-Actions secret masking issue noted in Phase 1.
...
- Healthchecks bootstrap and per-job check creation are documented as a new "Phase 2" section (steps 11–14) in `docs/monitoring.md`: first-boot `REGISTRATION_OPEN` flow, the recommended grace windows per check, the Healthchecks → `alert_sink` webhook integration template (`{"summary": "Missed heartbeat: $NAME", "severity": "critical", "source": "healthchecks/$NAME"}`), the acceptance checklist, individual-heartbeat pause via either the UI or a non-`http` SSM value, and a troubleshooting block covering the chown / EFS access-point fallback, the ping-URL caching behavior on Lambda cold start vs ECS task start (and the `--force-new-deployment` workaround for the latter), and the `var.backup`-must-be-true interaction with `backup_heartbeat`.
