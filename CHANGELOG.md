...
- `docker/healthchecks/Dockerfile` — thin wrapper over `healthchecks/healthchecks:v3.10`. Added to the docker workflow build matrix and to the ECR `extra_repositories` list. The `v3.10` upstream entrypoint runs `manage.py migrate` then `uwsgi` with no `chown` step, so unlike Kuma the task definition does not need to bypass the entrypoint; the operator runbook flags the pattern in case a future release reintroduces a chown shim.
