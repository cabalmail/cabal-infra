...
- **Resizable panes + address-rail polish.**
  ...
  - **Address rail polish.**
    ...
    - Comment-on-hover tooltip: rows that have a DynamoDB `comment` set get `data-comment={comment}` and a `::after` bubble below the row (token-styled, `--shadow-menu`, ellipsis at row width, 200 ms hover delay). The native `title=` attribute is suppressed when a comment exists so the browser tooltip doesn't race with the CSS one; rows without a comment keep `title=address` as a truncation fallback.
    - Auto-scroll on new-address success — restored the pre-redesign behavior. `onRequested` records the new address in a `pendingScroll` state bag; an effect watching `[addresses, pendingScroll]` waits until the refreshed list lands in state, then `querySelector`s the row inside the scoped list ref (`CSS.escape` to handle `@` / `.` / `+` in addresses) and calls `scrollIntoView({ block: 'nearest', behavior: 'smooth' })`. `block: 'nearest'` no-ops when the row is already visible, so we only scroll when needed.
    - Click-to-filter is now a toggle. `handleSelect` compares against `selectedAddress`; clicking the active row calls `onSelectAddress(null)` to clear the filter, while clicking any other row sets it as before. Keyboard `Enter` follows the same path. `Email/index.jsx`'s `selectAddress` already accepts `null` (it just writes through to `addressFilter`), so no change there.
- **Request modal restyled against the redesign tokens.** The "New address" form (used in the rail modal and the admin Addresses page) was rendering with legacy CSS bleeding through — black input fields, a dark-red `.default` Request button (from the `body .highlight, body .active, body .default { ... !important }` rule), and grey legacy `body button` chrome on Random / Clear. Rebuild:
  - `Request.jsx`: dropped the legacy `id="amphora"` / `id="dot"` / `className="default"` hooks and the auto-submit `<button>` defaults. Each input, separator, and button now carries a BEM-style `request__*` class. The submit button is `type="button"` and the empty-domain placeholder is now `<option value="">` so React doesn't warn about a controlled select with no matching option.
  - `Request.css` rewritten end-to-end against the Stately tokens — `--surface` / `--border` / `--ink` inputs with focus ring (`--accent` border + `accent`-soft 2px box-shadow), accent-fg primary `Request` button matching the rail's "New message" CTA, outlined secondary `Random` / `Clear` buttons, uppercase 11px legends keyed off `--ink-quiet`, and a custom-drawn caret on the domain `<select>`. Selectors are scoped under `.request` so specificity beats the legacy `body button` / `body input` rules in `AppLight.css` / `AppDark.css` without resorting to `!important`. The pulsing-while-sending animation is preserved as `request-ripple` on `.request__submit.sending`.
  - Dropped the vestigial `.requestVisible` / `.requestHidden` classes (and the `showRequest` prop) — they were a leftover max-height / padding-top transition pair from when Request was an inline expander, but the modal scrim and the admin-page wrapper both already mount/unmount the form. The transition could fire on first paint and snap shorter on the first hover-induced reflow, producing a visible "squish" before the form settled at content height.
