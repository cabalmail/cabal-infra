...
# --- IMAP connection pooling (Phase 7 / Layer 1.5) ---------------------------
...
def _new_imap_client(host, user, timeout=None):
...
class PooledImapClient:
    '''Transparent wrapper returned by get_imap_client when pooling is on.
...
def get_imap_client(host, user, folder, read_only=False):
    ...
    folder exactly as it always has, returning a bare IMAPClient. With pooling on
    it returns a PooledImapClient whose logout() checks the connection back into
    a per-(host, user) pool for reuse by the next warm invocation.'''
    ...
    if not POOL_ENABLED:
        ...
        return client
    return _get_pooled_imap_client(host, user, folder, read_only)


def _get_pooled_imap_client(host, user, folder, read_only):
    '''Pooled variant of get_imap_client: reuse a warm session if one is live,
    otherwise reconnect. The re-SELECT both sets the requested mailbox (a reused
    connection may have been checked in on a different folder -- it is never safe
    to trust its last SELECT) and serves as a liveness probe.'''
    key = (host, user)
    ...
    client = _new_imap_client(host, user, POOL_SOCKET_TIMEOUT)
    try:
        client.select_folder(folder, read_only)
    except Exception:  # pylint: disable=broad-except
